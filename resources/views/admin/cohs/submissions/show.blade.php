<x-layouts.admin header="Submission #{{ $submission->id }}">
    ...
    <div class="admin-page-wide max-w-3xl space-y-6">
        ...
        <form method="post" action="{{ route('admin.cohs.submissions.update', $submission) }}" class="admin-card flex flex-wrap items-center gap-4 p-4 sm:p-5">
            @csrf
            @method('PATCH')
        ...
        <p class="text-sm"><a href="{{ route('admin.cohs.submissions.index') }}" class="admin-link">← All submissions</a></p>
    </div>
</x-layouts.admin>
