    ...
    <div class="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-2 text-xs sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-x-6 sm:gap-y-1 sm:px-6 sm:text-sm lg:px-8">
        ...
        <div class="flex flex-wrap items-center gap-2 self-start sm:self-auto">
            ...
            >
                ...
                <?php echo e(__('College home')); ?>

            </a>
            <a
                href="<?php echo e($portalUrl); ?>"
            ...
            >
                <?php echo e($topBar['portal_label'] ?? 'Downloads & forms'); ?>
...
>
    <div class="relative mx-auto flex max-w-7xl items-center justify-between gap-2 overflow-visible px-4 py-2.5 sm:px-6 sm:py-3 lg:px-8 lg:py-3">
        <a
            ...
            <?php if($isSchoolLanding): ?> aria-current="page" <?php endif; ?>
            @click="openMega = null"
        >
            <img
                src="<?php echo e($brandLogo); ?>"
                alt="<?php echo e($school->name); ?> logo"
                width="56"
                height="56"
                class="h-11 w-11 shrink-0 rounded-full object-cover object-center sm:h-12 sm:w-12"
                decoding="async"
            >
            <span class="h-10 w-[3px] shrink-0 bg-thc-navy/35" aria-hidden="true"></span>
            <span class="min-w-0 leading-tight">
                <span class="block truncate text-sm font-semibold tracking-tight text-thc-navy group-hover:text-thc-royal"><?php echo e($school->name); ?></span>
                <span class="hidden truncate text-xs text-thc-text/65 sm:block"><?php echo e(config('tenwek.name')); ?></span>
            </span>
        </a>
        ...
        <nav class="hidden min-w-0 flex-1 items-center justify-end gap-0.5 py-1 lg:flex lg:flex-wrap lg:justify-center lg:gap-x-0.5 lg:gap-y-2 lg:px-2 xl:px-4" aria-label="College of Health Sciences">
            <?php $__currentLoopData = $mainNav; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if(! empty($item['mega_id']) && ! empty($item['children'])): ?>
                    <div class="relative shrink-0">
                        <button
                            ...
                            @click.stop="toggleMega('<?php echo e($item['mega_id']); ?>')"
                            :class="(openMega === '<?php echo e($item['mega_id']); ?>' || <?php echo \Illuminate\Support\Js::from($navItemActive($item))->toHtml() ?>) ? 'bg-thc-navy/10 text-thc-navy font-semibold ring-1 ring-thc-navy/18' : ''"
                            :aria-expanded="(openMega === '<?php echo e($item['mega_id']); ?>').toString()"
                            aria-haspopup="true"
                        >
                            <?php echo e($item['label']); ?>

                            <svg class="h-4 w-4 shrink-0 opacity-60 transition" :class="{ 'rotate-180': openMega === '<?php echo e($item['mega_id']); ?>' }" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                        </button>
                    </div>
                <?php else: ?>
                    <?php
                    ...
                    ?>
                    <a
                        href="<?php echo e($slug ? $page($slug) : '#'); ?>"
                        class="shrink-0 rounded-lg px-3 py-2 text-sm font-medium transition <?php echo e($navActive ? 'bg-thc-navy/10 text-thc-navy font-semibold ring-1 ring-thc-navy/18' : 'text-thc-navy/90 hover:bg-thc-navy/[0.06] hover:text-thc-royal'); ?>"
    ...
    <?php $__currentLoopData = $mainNav; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php if(! empty($item['mega_id']) && ! empty($item['children'])): ?>
            <div
                ...
                x-show="openMega === '<?php echo e($item['mega_id']); ?>'"
                    ...
                    <div class="grid gap-10 md:grid-cols-2">
                        <div>
                            ...
                            <p class="mt-3 max-w-md text-sm leading-relaxed text-thc-text/90">
                                <?php echo e($school->excerpt ?? $school->tagline ?? config('tenwek.tagline')); ?>
    ...
    >
        <div class="space-y-1 px-4 py-4">
            <?php $__currentLoopData = $mainNav; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if(! empty($item['mega_id']) && ! empty($item['children'])): ?>
                    <?php $mId = $item['mega_id']; ?>
                    <button
                        type="button"
                        class="flex w-full items-center justify-between rounded-lg px-3 py-3 text-left text-sm font-semibold text-thc-navy hover:bg-thc-navy/[0.06] hover:text-thc-royal <?php echo e($navItemActive($item) ? 'bg-thc-navy/10 font-semibold text-thc-navy ring-1 ring-inset ring-thc-navy/18' : ''); ?>"
                        data-mobile-section="<?php echo e($mId); ?>"
                    ...
                    <div x-show="mobileExpanded === <?php echo json_encode($mId, 15, 512) ?>" class="pl-2" style="display: none;">
                        <?php $__currentLoopData = $item['children']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $child): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <a
                                href="<?php echo e($childHref); ?>"
                                <?php if($childNewTab): ?> target="_blank" rel="noopener noreferrer" <?php endif; ?>
                                class="block rounded-lg px-3 py-2.5 text-sm <?php echo e($childNavActive ? 'bg-thc-navy/10 font-semibold text-thc-navy' : 'text-thc-navy/90 hover:bg-thc-navy/[0.06] hover:text-thc-royal'); ?>"
                                <?php if($childNavActive): ?> aria-current="page" <?php endif; ?>
                ...
                <?php else: ?>
                    ...
                    ?>
                    <a
                        href="<?php echo e($topHref); ?>"
                        <?php if($topNewTab): ?> target="_blank" rel="noopener noreferrer" <?php endif; ?>
                        class="block rounded-lg px-3 py-3 text-sm font-semibold <?php echo e($navActive ? 'bg-thc-navy/10 text-thc-navy ring-1 ring-inset ring-thc-navy/18' : 'text-thc-navy/90 hover:bg-thc-navy/[0.06] hover:text-thc-royal'); ?>"
                        <?php if($navActive): ?> aria-current="page" <?php endif; ?>
                        @click="mobileNavOpen = false"
                    ><?php echo e($item['label']); ?></a>
                <?php endif; ?>
...
<?php /**PATH C:\projects\Tenwek Projects\tenwek-college\resources\views/components/layouts/partials/cohs-header.blade.php ENDPATH**/ ?>